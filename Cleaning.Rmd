...
df_clean8 <- df_clean7 %>% 
  filter(gender == "M") %>% 
  mutate(height_cm = if_else(height_null == 1, 178.6, height_cm),
         weight_kg = if_else(weight_null == 1, 106.1, weight_kg),
         BMI = if_else(bmi_null == 1, 33.7, BMI))

df_clean8 <- df_clean8 %>% 
  filter(gender == "F") %>% 
  mutate(height_cm = if_else(height_null == 1, 165,4, height_cm),
         weight_kg = if_else(weight_null == 1, 87.6, weight_kg),
         BMI = if_else(bmi_null == 1, 33.7, BMI))
